--CRICAO DO BANCO
create table estado (id_estado smallint primary key,nome_estado varchar(25),sigla char(2));
...
create table indicativos_estado(id_estado smallint, ano smallint, tMortalidade_estado numeric(2) CHECK(tMortalidade_estado >= 0), tIDH numeric(3)CHECK(tIDH >=0 and tIDH <= 1),tAnalfabetismo_estado numeric(2) check(tAnalfabetismo_estado >= 0), tRendaPercapita_estado numeric(2) check (tRendaPercapita_estado >= 0), primary key (id_estado,ano), foreign key (id_estado) references estado(id_estado) );
